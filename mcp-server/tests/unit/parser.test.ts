...
describe('Markdown Parser', () => {
  ...
  describe('Endpoint Extraction', () => {
    it('should extract operation name correctly', async () => {
      const result = await parseMarkdownFile('../../docs/api/customer.md');
      const getCustomersEndpoint = result.endpoints.find(
    ...
    it('should extract description correctly', async () => {
      ...
      expect(getCustomersEndpoint?.description).toContain(
    ...
    it('should extract permission correctly', async () => {
      ...
      expect(getCustomersEndpoint?.permission).toContain(
    ...
    it('should extract path parameters correctly', async () => {
      const result = await parseMarkdownFile('../../docs/api/customer.md');
      const getCustomerByIdEndpoint = result.endpoints.find(
      ...
      const idParam = getCustomerByIdEndpoint?.parameters.find(
        (p) => p.name === 'id'
      );
    ...
    it('should extract query parameters correctly', async () => {
      ...
      const pageParam = getCustomersEndpoint?.parameters.find(
        (p) => p.name === 'page'
      );
    ...
    it('should handle endpoints with no parameters', async () => {
      const result = await parseMarkdownFile('../../docs/api/customer.md');
      const deleteCustomerEndpoint = result.endpoints.find(
      ...
      const idParam = deleteCustomerEndpoint?.parameters.find(
        (p) => p.name === 'id'
      );
      expect(idParam).toBeDefined();
    });
    ...
    it('should extract request body parameters for POST endpoints', async () => {
      const result = await parseMarkdownFile('../../docs/api/customer.md');
      const createCustomerEndpoint = result.endpoints.find(
  ...
  describe('Response Parsing', () => {
    ...
    it('should handle multiple responses per endpoint', async () => {
      ...
      const successResponse = createCustomerEndpoint?.responses.find(
        (r) => r.statusCode === 200
      );
      const errorResponse = createCustomerEndpoint?.responses.find(
        (r) => r.statusCode === 422
      );
    ...
    it('should handle responses without examples', async () => {
      ...
      const notFoundResponse = getCustomerByIdEndpoint?.responses.find(
        (r) => r.statusCode === 404
      );
  ...
  describe('Parameter Table Parsing', () => {
    it('should parse parameter names correctly', async () => {
      ...
      const paramNames =
        getCustomersEndpoint?.parameters.map((p) => p.name) || [];
    ...
    it('should parse parameter types correctly', async () => {
      ...
      const idParam = getCustomersEndpoint?.parameters.find(
        (p) => p.name === 'id'
      );
    ...
    it('should handle empty parameter descriptions', async () => {
      ...
      const emailParam = getCustomersEndpoint?.parameters.find(
        (p) => p.name === 'email'
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle endpoints with multiple responses', async () => {
      ...
      expect(createCustomerEndpoint?.responses.length).toBeGreaterThanOrEqual(
        2
      );
    });
    ...
    it('should handle endpoints with no description', async () => {
      ...
      const endpointsWithNoDescription = result.endpoints.filter(
        (e) => e.description === ''
      );
  ...
  describe('Different Resource Types', () => {
    ...
    it('should parse ticket endpoints with nested paths', async () => {
      const result = await parseMarkdownFile('../../docs/api/ticket.md');
      const getTicketCommentsEndpoint = result.endpoints.find(
        (e) => e.path === '/tickets/{id}/comments'
      );
    ...
    it('should parse ticket endpoints with complex query parameters', async () => {
      const result = await parseMarkdownFile('../../docs/api/ticket.md');
      const getTicketsEndpoint = result.endpoints.find(
        (e) => e.operation === 'Get Tickets'
      );

      expect(getTicketsEndpoint?.parameters.length).toBeGreaterThan(0);

      const statusParam = getTicketsEndpoint?.parameters.find(
        (p) => p.name === 'status'
      );
