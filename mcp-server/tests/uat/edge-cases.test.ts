...
import { generateEdgeCaseQueries, generateInvalidQueryParams } from '../utils/data-generators';
...
const concurrentRequestEdgeCases = [
  ...
  },
  {
    ...
    queries: ['get customer', 'get customer with parameters', 'create customer', 'update customer'],
    expectedBehavior: 'Should handle all requests correctly',
    shouldPass: true
  },
  {
...
describe('Edge Case Tests - Query Edge Cases', () => {
  ...
  test.each(queryEdgeCases)('$description', async ({ id, description, query, expectedBehavior, shouldPass }) => {
  ...
  test('should handle all query edge cases without crashing', () => {
    const crashCount = results.filter(r => r.error && r.error.includes('crash')).length;
...
describe('Edge Case Tests - Response Edge Cases', () => {
  ...
  test.each(responseEdgeCases)('$description', async ({ id, description, query, expectedBehavior, shouldPass }) => {
    const startTime = Date.now();
    let result: any;
    let error: string | undefined;
...
describe('Edge Case Tests - Tool Execution Edge Cases', () => {
  ...
  test.each(toolExecutionEdgeCases)('$description', async ({ id, description, query, expectedBehavior, shouldPass }) => {
    const startTime = Date.now();
    let result: any;
    let error: string | undefined;
...
describe('Edge Case Tests - Error Handling Edge Cases', () => {
  ...
  test.each(errorHandlingEdgeCases.filter(c => !c.simulateTimeout && !c.simulateUnauthorized && !c.simulateRateLimit && !c.simulateInternalError && !c.simulateDbError && !c.simulateConcurrentMod))(
    '$description',
  ...
  test('should provide clear error messages', () => {
    ...
    const unclearErrors = errorCases.filter(r => !r.response.message || r.response.message.length < 10);
...
describe('Edge Case Tests - Concurrent Request Edge Cases', () => {
  ...
  test('should handle requests with varying complexity', async () => {
    const queries = ['get customer', 'get customer with parameters', 'create customer', 'update customer'];
    const startTime = Date.now();
    const promises = queries.map(q => server.handleQuery(q));
    const results = await Promise.all(promises);
    const duration = Date.now() - startTime;
  ...
  test('should not have race conditions in concurrent requests', async () => {
    const queries = ['get customer', 'get ticket', 'get invoice', 'get customer', 'get ticket'];
    const promises = queries.map(q => server.handleQuery(q));
    const results = await Promise.all(promises);
...
export function generateEdgeCaseReport(results: EdgeCaseTestResult[]): string {
  ...
  Object.keys(report.byCategory).forEach(category => {
    ...
    report.byCategory[category] = {
      ...
      passed: categoryResults.filter((r: EdgeCaseTestResult) => r.passed).length,
      failed: categoryResults.filter((r: EdgeCaseTestResult) => !r.passed).length,
      passRate: categoryResults.filter((r: EdgeCaseTestResult) => r.passed).length / categoryResults.length,
      averageDuration: categoryResults.reduce((sum: number, r: EdgeCaseTestResult) => sum + r.duration, 0) / categoryResults.length
    };
  });
