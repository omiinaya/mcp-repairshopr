 ...
 */
function getAllEndpoints(index: {
  resources: Record<string, ApiEndpoint[]>;
}): ApiEndpoint[] {
  const endpoints: ApiEndpoint[] = [];

  for (const [resourceName, resourceEndpoints] of Object.entries(
    index.resources
  )) {
...
async function buildEmbeddings(): Promise<void> {
  ...
  console.log(
    `Loaded metadata index with ${Object.keys(index.resources).length} resources\n`
  );
  ...
  for (const query of testQueries) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      console.log(
        `  ${i + 1}. [Score: ${result.score.toFixed(4)}] ${result.metadata.resource} - ${result.metadata.method} ${result.metadata.path}`
      );
    }
  }
