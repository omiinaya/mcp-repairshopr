...
function generateIssues(
  ...
  for (const [key, value] of Object.entries(metrics)) {
    ...
    if (threshold && value < threshold) {
      ...
      if (diff > 10) {
        issues.push(
          `${key}: ${value.toFixed(1)}% (threshold: ${threshold}%, gap: ${diff.toFixed(1)}%)`
        );
      } else {
        issues.push(
          `${key}: ${value.toFixed(1)}% (threshold: ${threshold}%, gap: ${diff.toFixed(1)}%) - needs improvement`
        );
      }
    }
  }
...
function generateRecommendations(report: AccuracyReport): string[] {
  ...
  if (
    report.parsingAccuracy.status === 'fail' ||
    report.parsingAccuracy.status === 'warning'
  ) {
    if (
      report.parsingAccuracy.metadataAccuracy <
      ACCURACY_THRESHOLDS.parsing.metadataAccuracy
    ) {
      recommendations.push(
        'Improve metadata extraction accuracy by enhancing header parsing logic'
      );
    }
    if (
      report.parsingAccuracy.parameterAccuracy <
      ACCURACY_THRESHOLDS.parsing.parameterAccuracy
    ) {
      recommendations.push(
        'Enhance parameter extraction to handle edge cases and malformed tables'
      );
    }
    if (
      report.parsingAccuracy.responseAccuracy <
      ACCURACY_THRESHOLDS.parsing.responseAccuracy
    ) {
      recommendations.push(
        'Improve response extraction to better handle JSON examples and descriptions'
      );
    }
  }
  ...
  if (
    report.searchRelevance.status === 'fail' ||
    report.searchRelevance.status === 'warning'
  ) {
    if (
      report.searchRelevance.semanticAccuracy <
      ACCURACY_THRESHOLDS.search.semanticAccuracy
    ) {
      recommendations.push(
        'Improve semantic search by enhancing vector embeddings and similarity calculations'
      );
    }
    if (
      report.searchRelevance.keywordAccuracy <
      ACCURACY_THRESHOLDS.search.keywordAccuracy
    ) {
      recommendations.push(
        'Enhance keyword search by improving tokenization and matching algorithms'
      );
    }
    if (
      report.searchRelevance.rankingAccuracy <
      ACCURACY_THRESHOLDS.search.rankingAccuracy
    ) {
      recommendations.push(
        'Optimize result ranking by fine-tuning relevance scoring weights'
      );
    }
  }
  ...
  if (
    report.parameterExtraction.status === 'fail' ||
    report.parameterExtraction.status === 'warning'
  ) {
    if (
      report.parameterExtraction.nameAccuracy <
      ACCURACY_THRESHOLDS.parameterExtraction.nameAccuracy
    ) {
      recommendations.push(
        'Improve parameter name extraction by handling special characters and formatting'
      );
    }
    if (
      report.parameterExtraction.typeAccuracy <
      ACCURACY_THRESHOLDS.parameterExtraction.typeAccuracy
    ) {
      recommendations.push(
        'Enhance parameter type extraction to recognize all valid types'
      );
    }
  }
  ...
  if (
    report.responseFormatting.status === 'fail' ||
    report.responseFormatting.status === 'warning'
  ) {
    if (
      report.responseFormatting.contextWindowAccuracy <
      ACCURACY_THRESHOLDS.responseFormatting.contextWindowAccuracy
    ) {
      recommendations.push(
        'Improve context window management by optimizing token estimation'
      );
    }
    if (
      report.responseFormatting.summarizationAccuracy <
      ACCURACY_THRESHOLDS.responseFormatting.summarizationAccuracy
    ) {
      recommendations.push(
  ...
  if (
    report.manualQueryComparison.status === 'fail' ||
    report.manualQueryComparison.status === 'warning'
  ) {
    if (
      report.manualQueryComparison.queryUnderstandingAccuracy <
      ACCURACY_THRESHOLDS.manualQueryComparison.queryUnderstandingAccuracy
    ) {
      recommendations.push(
        'Improve query understanding by enhancing natural language processing'
      );
    }
    if (
      report.manualQueryComparison.queryExpansionAccuracy <
      ACCURACY_THRESHOLDS.manualQueryComparison.queryExpansionAccuracy
    ) {
      recommendations.push(
        'Enhance query expansion to better match user intent'
      );
    }
  }
  ...
  if (report.summary.overallAccuracy < 90) {
    recommendations.push(
      'Overall accuracy is below 90%. Consider comprehensive review of all components'
    );
  }
...
function generateAccuracyReport(
  ...
  const parameterExtractionPassRate = calculatePassRate(
    parameterExtractionResults
  );
  const responseFormattingPassRate = calculatePassRate(
    responseFormattingResults
  );
  const manualQueryComparisonPassRate = calculatePassRate(
  ...
  const parsingStatus = determineStatus(
    parsingPassRate,
    ACCURACY_THRESHOLDS.parsing.overallAccuracy
  );
  ...
  const searchStatus = determineStatus(
    searchPassRate,
    ACCURACY_THRESHOLDS.search.overallRelevance
  );
...
function formatReportAsMarkdown(report: AccuracyReport): string {
  ...
  lines.push('');
  lines.push(
    `**Overall Accuracy:** ${report.summary.overallAccuracy.toFixed(1)}%`
  );
  ...
  lines.push('');
  lines.push(
    `**Overall:** ${report.parsingAccuracy.overallAccuracy.toFixed(1)}% (${report.parsingAccuracy.status})`
  );
  lines.push(
    `- Metadata: ${report.parsingAccuracy.metadataAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Parameters: ${report.parsingAccuracy.parameterAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Responses: ${report.parsingAccuracy.responseAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Edge Cases: ${report.parsingAccuracy.edgeCaseAccuracy.toFixed(1)}%`
  );
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `**Overall:** ${report.searchRelevance.overallRelevance.toFixed(1)}% (${report.searchRelevance.status})`
  );
  lines.push(
    `- Semantic: ${report.searchRelevance.semanticAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Keyword: ${report.searchRelevance.keywordAccuracy.toFixed(1)}%`
  );
  lines.push(`- Hybrid: ${report.searchRelevance.hybridAccuracy.toFixed(1)}%`);
  lines.push(
    `- Ranking: ${report.searchRelevance.rankingAccuracy.toFixed(1)}%`
  );
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `**Overall:** ${report.parameterExtraction.overallAccuracy.toFixed(1)}% (${report.parameterExtraction.status})`
  );
  ...
  lines.push(
    `- Required: ${report.parameterExtraction.requiredAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Descriptions: ${report.parameterExtraction.descriptionAccuracy.toFixed(1)}%`
  );
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `**Overall:** ${report.responseFormatting.overallAccuracy.toFixed(1)}% (${report.responseFormatting.status})`
  );
  lines.push(
    `- Markdown: ${report.responseFormatting.markdownAccuracy.toFixed(1)}%`
  );
  ...
  lines.push(
    `- Context Window: ${report.responseFormatting.contextWindowAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Summarization: ${report.responseFormatting.summarizationAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Prioritization: ${report.responseFormatting.prioritizationAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Structure: ${report.responseFormatting.structureAccuracy.toFixed(1)}%`
  );
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `**Overall:** ${report.manualQueryComparison.overallAccuracy.toFixed(1)}% (${report.manualQueryComparison.status})`
  );
  lines.push(
    `- Query Understanding: ${report.manualQueryComparison.queryUnderstandingAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Query Expansion: ${report.manualQueryComparison.queryExpansionAccuracy.toFixed(1)}%`
  );
  lines.push(
    `- Edge Cases: ${report.manualQueryComparison.edgeCaseAccuracy.toFixed(1)}%`
  );
  lines.push('');
  ...
  lines.push('### Parameter Extraction');
  lines.push(
    `- Overall: ${ACCURACY_THRESHOLDS.parameterExtraction.overallAccuracy}%`
  );
  lines.push(
    `- Names: ${ACCURACY_THRESHOLDS.parameterExtraction.nameAccuracy}%`
  );
  lines.push(
    `- Types: ${ACCURACY_THRESHOLDS.parameterExtraction.typeAccuracy}%`
  );
  lines.push(
    `- Required: ${ACCURACY_THRESHOLDS.parameterExtraction.requiredAccuracy}%`
  );
  lines.push(
    `- Descriptions: ${ACCURACY_THRESHOLDS.parameterExtraction.descriptionAccuracy}%`
  );
  lines.push('');

  lines.push('### Response Formatting');
  lines.push(
    `- Overall: ${ACCURACY_THRESHOLDS.responseFormatting.overallAccuracy}%`
  );
  lines.push(
    `- Markdown: ${ACCURACY_THRESHOLDS.responseFormatting.markdownAccuracy}%`
  );
  ...
  lines.push(
    `- Context Window: ${ACCURACY_THRESHOLDS.responseFormatting.contextWindowAccuracy}%`
  );
  lines.push(
    `- Summarization: ${ACCURACY_THRESHOLDS.responseFormatting.summarizationAccuracy}%`
  );
  lines.push(
    `- Prioritization: ${ACCURACY_THRESHOLDS.responseFormatting.prioritizationAccuracy}%`
  );
  lines.push(
    `- Structure: ${ACCURACY_THRESHOLDS.responseFormatting.structureAccuracy}%`
  );
  lines.push('');

  lines.push('### Manual Query Comparison');
  lines.push(
    `- Overall: ${ACCURACY_THRESHOLDS.manualQueryComparison.overallAccuracy}%`
  );
  lines.push(
    `- Query Understanding: ${ACCURACY_THRESHOLDS.manualQueryComparison.queryUnderstandingAccuracy}%`
  );
  lines.push(
    `- Query Expansion: ${ACCURACY_THRESHOLDS.manualQueryComparison.queryExpansionAccuracy}%`
  );
  lines.push(
    `- Edge Cases: ${ACCURACY_THRESHOLDS.manualQueryComparison.edgeCaseAccuracy}%`
  );
  lines.push('');
...
async function validateAccuracy(): Promise<void> {
  ...
  console.log(
    `Parsing accuracy tests completed with exit code: ${parsingOutput.exitCode}\n`
  );
  ...
  console.log(
    `Search relevance tests completed with exit code: ${searchOutput.exitCode}\n`
  );

  console.log('Running parameter extraction tests...');
  const parameterExtractionOutput = runJestTests(
    'tests/accuracy/parameter-extraction.test.ts'
  );
  const parameterExtractionResults = parseJestOutput(
    parameterExtractionOutput.stdout
  );
  console.log(
    `Parameter extraction tests completed with exit code: ${parameterExtractionOutput.exitCode}\n`
  );

  console.log('Running response formatting tests...');
  const responseFormattingOutput = runJestTests(
    'tests/accuracy/response-formatting.test.ts'
  );
  const responseFormattingResults = parseJestOutput(
    responseFormattingOutput.stdout
  );
  console.log(
    `Response formatting tests completed with exit code: ${responseFormattingOutput.exitCode}\n`
  );

  console.log('Running manual query comparison tests...');
  const manualQueryComparisonOutput = runJestTests(
    'tests/accuracy/manual-query-comparison.test.ts'
  );
  const manualQueryComparisonResults = parseJestOutput(
    manualQueryComparisonOutput.stdout
  );
  console.log(
    `Manual query comparison tests completed with exit code: ${manualQueryComparisonOutput.exitCode}\n`
  );
  ...
  console.log(
    `- Parsing: ${report.parsingAccuracy.overallAccuracy.toFixed(1)}% (${report.parsingAccuracy.status})`
  );
  console.log(
    `- Search: ${report.searchRelevance.overallRelevance.toFixed(1)}% (${report.searchRelevance.status})`
  );
  console.log(
    `- Parameter Extraction: ${report.parameterExtraction.overallAccuracy.toFixed(1)}% (${report.parameterExtraction.status})`
  );
  console.log(
    `- Response Formatting: ${report.responseFormatting.overallAccuracy.toFixed(1)}% (${report.responseFormatting.status})`
  );
  console.log(
    `- Manual Query Comparison: ${report.manualQueryComparison.overallAccuracy.toFixed(1)}% (${report.manualQueryComparison.status})`
  );
  console.log('');
  ...
  if (report.summary.status === 'fail') {
    ...
    console.warn(
      'Accuracy validation completed with WARNINGS. See report for details.'
    );
    process.exit(0);
  } else {
    console.log(
      'Accuracy validation PASSED. All metrics meet or exceed thresholds.'
    );
    process.exit(0);
  }
}
