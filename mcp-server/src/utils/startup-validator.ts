...
const DEFAULT_CONFIG: ValidationConfig = {
  requiredFiles: ['data/metadata-index.json', 'config/default.json'],
  optionalFiles: ['config/production.json', 'data/vector-index.json'],
    ...
    for (const file of this.config.requiredFiles) {
      ...
      try {
        ...
        if (
          ...
          fileSizeMB > this.config.maxFileSizeMB
        ) {
          warnings.push(
            `File ${file} is very large (${fileSizeMB.toFixed(2)} MB)`
          );
        }
  ...
  private validateEnvironment(): { errors: string[]; warnings: string[] } {
    ...
    if (process.env.LOG_LEVEL) {
      ...
      if (!validLevels.includes(process.env.LOG_LEVEL.toLowerCase())) {
        warnings.push(
          `Invalid LOG_LEVEL: ${process.env.LOG_LEVEL}. Using default.`
        );
      }
    }
    ...
    try {
      ...
      if (!data.resources || typeof data.resources !== 'object') {
        ...
        if (resourceCount === 0) {
          ...
          warnings.push(
            `Metadata index contains only ${resourceCount} resources`
          );
        }
      }
      ...
      if (!data.endpoints || !Array.isArray(data.endpoints)) {
        ...
        if (data.endpoints.length === 0) {
          ...
          warnings.push(
            `Metadata index contains only ${data.endpoints.length} endpoints`
          );
        }
      }
      ...
      if (!data.parsedAt) {
        ...
        if (ageDays > 30) {
          warnings.push(
            `Metadata index is ${ageDays.toFixed(0)} days old. Consider regenerating.`
          );
        }
      }
    } catch (error) {
