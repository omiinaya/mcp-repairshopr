 ...
 */
export async function parseMarkdownFile(filePath: string): Promise<ApiDocument> {
...
function extractResourceName(lines: string[]): string {
  const headerLine = lines.find(line => line.startsWith('# RepairShopr API Documentation -'));
...
function parseEndpoints(lines: string[], resourceName: string): ApiEndpoint[] {
  ...
  for (let i = 0; i < endpointIndices.length; i++) {
    ...
    const endIndex = i < endpointIndices.length - 1 ? endpointIndices[i + 1] : lines.length;
    ...
    try {
      ...
      console.warn(`Failed to parse endpoint at line ${startIndex + 1}:`, error);
    }
  }
...
function extractDescription(lines: string[]): string {
  const endpointIndex = lines.findIndex(line => line.startsWith('**Endpoint:**'));
 ...
 */
function extractEndpointInfo(lines: string[]): { method: string; path: string } {
...
function extractPermission(lines: string[]): string {
  const permissionLine = lines.find(line => line.startsWith('**Required Permission:**'));
...
function extractParameters(lines: string[]): ApiParameter[] {
  ...
  const queryParamIndex = lines.findIndex(line => line === '**Query Parameters:**');
  ...
  const pathParamIndex = lines.findIndex(line => line === '**Path Parameters:**');
  if (pathParamIndex !== -1) {
 ...
 */
function parseParameterTable(lines: string[], startIndex: number, paramType: 'query' | 'path' | 'body'): ApiParameter[] {
  const parameters: ApiParameter[] = [];
  ...
  if (tableStartIndex >= lines.length || !lines[tableStartIndex].startsWith('|')) {
    return parameters;
  }
  ...
  // Parse data rows
  while (tableStartIndex < lines.length && lines[tableStartIndex].startsWith('|')) {
...
function extractRequestBody(lines: string[]): ApiParameter[] | undefined {
  const requestBodyIndex = lines.findIndex(line => line === '**Request Body:**');
...
function extractResponses(lines: string[]): ApiResponse[] {
  ...
  for (let i = 0; i < responseIndices.length; i++) {
    ...
    const endIndex = i < responseIndices.length - 1 ? responseIndices[i + 1].index : lines.length;
...
function extractResponseExample(lines: string[]): any {
  ...
  const codeBlockEnd = lines.findIndex((line, index) => index > codeBlockStart && line.startsWith('```'));
